"""
Docker-compatible Evaluation Script for Fine-tuned Text-to-SQL Models.

Evaluates GPT-2 and TinyLlama models trained on geography and atis datasets.
Uses /app paths for Docker container execution.
"""
...
TRAINED_MODELS = {
    ...
    "gpt2-advising": {
        "type": "gpt2",
        "path": TRAINING_RESULTS_DIR / "gpt2-advising" / "final",
        "database": "advising"
    },
    "tinyllama-geography": {
        "type": "tinyllama",
    ...
    "tinyllama-advising": {
        "type": "tinyllama",
        "path": TRAINING_RESULTS_DIR / "tinyllama-advising" / "final",
        "database": "advising"
    }
}
...
def load_tinyllama_model(model_path):
    ...
    # Load tokenizer from base model (adapter tokenizer config is corrupted)
...
def generate_sql(model, tokenizer, question, schema, model_type):
    """Generate SQL for a question."""
    if model_type == "gpt2":
        prompt = f"Table: {schema}\nQuestion: {question}\nSQL:"
        sql_marker = "SQL:"
    else:
        # TinyLlama - use concise schema format
        prompt = f"### Schema:\n{schema}\n\n### Question:\n{question}\n\n### SQL:\n"
        sql_marker = "### SQL:"
    ...
    with torch.no_grad():
        if model_type == "tinyllama":
            # Beam search works better for TinyLlama
...
def evaluate_model(model_name, model, tokenizer, model_type, test_examples, db_config, num_samples=20):
    """Evaluate a single model."""
    print(f"\n{'='*60}")
    print(f"Evaluating: {model_name}")
    print(f"{'='*60}")
    
    # Use short schema for both models (concise works better for TinyLlama too)
    schema = db_config["schema_short"]
...
def run_all_evaluations(num_samples=20):
